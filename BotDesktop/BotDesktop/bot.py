...
class Bot(DesktopBot):
    ...
    def action(self, execution=None):
        if not self.page_initialized:
            # Verificação da imagem pixels.png para determinar se o site já está aberto
            if not self.check_image(r"W:\Git\Bot\BotCity\BotDesktop\BotDesktop\resources\pixels.png"):
                logging.info("A imagem 'pixels.png' não foi encontrada. Abrindo o site...")
                self.browse("https://play.pixels.xyz/")
                self.page_initialized = True
            else:
                logging.info("A página do Pixels já está aberta. Pulando a inicialização.")
        ...
        while True:
            ...
            self.click_until_found(["worldclick"], matching=0.75, on_fail=self.handle_not_found_worldclick)
            ...
            imagens_vaca = ["vacaalt1", "vacaalt2", "vacaalt3", "vacaalt4", "vacaalt5", "vacaalt6", "vacaalt7", "vacaalt8", "click1", "click2"]
            ...
            # Loop para realizar a verificação de "milk" e "leite1" três vezes
            while milk_check_attempts < 3:
                # Verifica se a imagem 'milk' ou 'leite1' está presente após clicar na vaca
                if self.find("milk", matching=0.75, waiting_time=1) or self.find("leite1", matching=0.75, waiting_time=1):
                    logging.info(f"Tentativa de verificação de 'milk' ou 'leite1': {milk_check_attempts + 1}")
                    # Incrementa o contador de tentativas bem-sucedidas de verificação de "milk" ou "leite1"
                    milk_check_attempts += 1

                    # Verifica se ocorreu um missclick após clicar em "milk" ou "leite1"
                    if self.check_missclick():
                        self.handle_missclick()
                    ...
                    # Verifica se o elemento "milk" ou "leite1" ainda estão presentes após clicar na vaca
                    if self.find("milk", matching=0.75, waiting_time=1) or self.find("leite1", matching=0.75, waiting_time=1):
                        logging.info("Elemento 'milk' ou 'leite1' ainda presente após clicar na vaca. Continuando para próxima tentativa...")
                        continue

                    break  # Sai do loop após clicar na vaca e verificar "milk" ou "leite1" com sucesso
                else:
                    if self.is_mouse_over_vaca():
                        logging.info("O mouse está sobre uma imagem da vaca.")
                        logging.info("Elemento 'milk' ou 'leite1' não encontrado. Continuando para 'moomuch'...")
                        # Verifica se a imagem 'moomuch' está presente
                        if self.find("moomuch", matching=0.75, waiting_time=1):
                            logging.info("Imagem 'moomuch' encontrada. Saindo do loop de verificação de 'milk' e 'leite1'.")

                start_time = time.time()  # Registra o tempo de início da ação de clicar em 'moomuch1'
                logging.info("Clicando em 'moomuch1'...")
                if not self.click_until_found(["moomuch1"], matching=0.75, waiting_time=2):  # Reduzindo o tempo de espera
                    logging.info("Acabou 'moomuch1'!!!.")
                    pyautogui.hotkey('ctrl', 'w')
                    pyautogui.hotkey('ctrl', 'w')
                    logging.info("Ação em 'moomuch1' concluída.")
                    return  # Termina a função action() se 'moomuch1' não for encontrado
                logging.info(f"Tempo para clicar em 'moomuch1': {time.time() - start_time:.2f} segundos. Procurando pela imagem 'moomuch1'.")

                start_time = time.time()  # Registra o tempo de início da ação de clicar em 'vaca' ou 'click'
                logging.info("Procurando")
                if not self.click_until_found(imagens_vaca, matching=0.75, waiting_time=1):  # Reduzindo o tempo de espera
                    logging.info("Click")
                    return  # Termina a função action() se 'vaca' ou 'click' não for encontrado
                logging.info(f"Tempo para clicar em 'vaca' ou 'click': {time.time() - start_time:.2f} segundos. Procurando por uma das imagens: {', '.join(imagens_vaca)}.")

                start_time = time.time()  # Registra o tempo de início da ação de clicar em 'moomuch3'
                logging.info("Clicando em 'moomuch3'...")
                if not self.click_until_found(["moomuch3", "moomuch3_alternative"], matching=0.85):  # Reduzindo o tempo de espera
                    logging.info("Acabou 'moomuch3'!!!.")
                    return  # Termina a função action() se 'moomuch3' não for encontrado
                logging.info(f"Tempo para clicar em 'moomuch3': {time.time() - start_time:.2f} segundos. Procurando pelas imagens: 'moomuch3', 'moomuch3_alternative'.")
    ...
    def is_mouse_over_vaca(self):
        return self.find("vacaalt1", matching=0.75, waiting_time=1) or \
    ...
    def check_image(self, image_path):
        try:
            pixels_logo_location = pyautogui.locateCenterOnScreen(image_path, confidence=0.60)
            if pixels_logo_location is not None:
                logging.info(f"A imagem '{image_path}' foi encontrada.")
                return True
            else:
                logging.info(f"A imagem '{image_path}' não foi encontrada.")
                return False
        except Exception as e:
            logging.error(f"Erro ao encontrar a imagem '{image_path}': {e}")
            return False
